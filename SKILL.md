...
5. 响应字段名中只要包含 `Deducted`，都表示扣量后的值，即该指标已经按一定扣量比例扣除，不是原始值。
6. 对外解释这类字段时，要明确写成“扣量后”或“扣除一定比例后的值”，不要和原始指标混用。
7. `query_personal_summary` / `query_product_summary` 返回中的 `queryStartDate` / `queryEndDate` 是展示字段；若与用户请求范围不完全一致，不要直接判定为筛选失效。
